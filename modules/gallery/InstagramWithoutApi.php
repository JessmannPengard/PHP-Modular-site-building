...
class Fetch
{
    ...
    public static function fetchById($array)
    {
        ...
        if (!isset($instagramId) || $instagramId == '') {
            $instagramId = '2890411760684296309';
        }
        ...
        if (
            ...
            if ($content) {
                ...
                if ($item && $image) {
                    ...
                    if ($base64images && $image['url'])
                        $newResult['image'] = base64_encode(file_get_contents(@$image['url']));
                        ...
                        for ($i = 0; $i < $counter; $i++) {
                            ...
                            if (
                                ...
                                for ($k = 0; $k < count($carouselNodes); $k++) {
                                    ...
                                    if ($base64imagesCarousel)
                                        $newItem['image'] = base64_encode(file_get_contents(@$carouselNodes[$k]['node']['display_url']));
                                    $carousel[] = $newItem;
                                }
                            ...
                            if ($base64images)
                                $newResult['image'] = base64_encode(file_get_contents(@$items[$i]['node']['display_url']));
                            ...
                            if (
                                ...
                                if ($base64videos)
                                    $newResult['video'] = base64_encode(file_get_contents(@$items[$i]['node']['video_url']));
    ...
    public static function fetchByTag($array)
    {
        ...
        if (!$group || $group != 'recent' && $group != 'top')
            $group = 'recent';
        @$maxImages = $array['maxImages'] && is_numeric($array['maxImages']) && $array['maxImages'] <= 12 ? $array['maxImages'] : false;
        @$header = $array['header'] ? $array['header'] : false;
        @$file = $array['file'] ? $array['file'] : 'instagram-cache.json';
                                ...
                                if ($counter < $maxImages) {
                                    ...
                                    if (
                                        ...
                                        for ($x = 0; $x < count($media['carousel_media']); $x++) {
                                            ...
                                            if ($base64imagesCarousel)
                                                $newItem['image'] = base64_encode(file_get_contents(@$media['carousel_media'][$x]['image_versions2']['candidates'][0]['url']));
                                            $carousel[] = $newItem;
                                    ...
                                    if ($base64images)
                                        $newResult['image'] = base64_encode(file_get_contents(@$imageUrl));
                                    ...
                                    if (
                                        ...
                                        if ($base64videos)
                                            $newResult['video'] = base64_encode(file_get_contents($media['video_versions'][0]['url']));
                                    }
